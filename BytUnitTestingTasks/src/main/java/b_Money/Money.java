...
public class Money implements Comparable {
	...
	//I realised that it was supposed to be double after creating all methods so that i cant chnage now
...
//equals method changed
	...
	public Money add(Money other) {
		int money1 = (int) (other.getAmount() * other.getCurrency().getRate());
	...
	public int compareTo(Object other) {
		if (!(other instanceof Money)) {
			throw new IllegalArgumentException("Comparison with non-Money object");
		}

		Money otherMoney = (Money) other;
		return Integer.compare(this.amount, otherMoney.amount);
	}
	//compare to method changed
